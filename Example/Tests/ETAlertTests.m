...
- (void)testAlert {
    ...
    [tester ne_et_asyncTapViewWithAccessibilityLabel:@"系统Alert"];
    ...
    NE_ET_Test_WaitForTimeAtVTreeGenerateWithCondition(3, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_VIEW spm:show_alert_btn_spm].count > 0;
    });
    NE_ET_Test_WaitForTime(0.5);
    
    NEEventTracingVTree *VTree = logComing.lastVTree;
    NEEventTracingVTreeNode *page_alert_node = [self _findNodeBySPM:@"page_alert" inVTree:VTree];
    ...
    // 点击 show alert 按钮
    [tester ne_et_asyncTapViewWithAccessibilityLabel:@"ShowAlert"];
    NE_ET_Test_WaitForTimeAtVTreeGenerateWithCondition(3, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_CLCK spm:show_alert_btn_spm].count > 0;
    });
    
    VTree = logComing.lastVTree;
    NEEventTracingVTreeNode *show_alert_btn_node = [self _findNodeBySPM:show_alert_btn_spm inVTree:VTree];
    ...
    NE_ET_Test_WaitForTimeAtVTreeGenerateWithCondition(3, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_P_VIEW spm:alert_spm].count > 0;
    });

    NE_ET_Test_WaitForTime(0.5);
    
    VTree = logComing.lastVTree;
    NSArray<NSDictionary *> *alert_pv_logJsons = [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_P_VIEW spm:alert_spm];
    NSArray<NSDictionary *> *alert_btn_ok_ev_logJsons = [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_VIEW spm:alert_ok_btn_spm];
    NSArray<NSDictionary *> *alert_btn_cancel_ev_logJsons = [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_VIEW spm:alert_cancel_btn_spm];
    ...
    NEEventTracingVTreeNode *alert_node = [self _findNodeBySPM:alert_spm inVTree:VTree];
    NEEventTracingVTreeNode *alert_btn_ok_node = [self _findNodeBySPM:alert_ok_btn_spm inVTree:VTree];
    NEEventTracingVTreeNode *alert_btn_cancel_node = [self _findNodeBySPM:alert_cancel_btn_spm inVTree:VTree];
    ...
    XCTAssertTrue(alert_btn_ok_node.buildinEventLogDisableStrategy & NEETNodeBuildinEventLogDisableStrategyImpress);  // 禁止了曝光埋点
    ...
    XCTAssertTrue(alert_btn_cancel_node.buildinEventLogDisableStrategy & NEETNodeBuildinEventLogDisableStrategyImpress);  // 禁止了曝光埋点
    XCTAssertTrue([alert_btn_cancel_node.nodeParams.allKeys containsObject:@"alert_btn_cancel_p_key"]);     // 对象参数
    
    // 点击 OK
    logComing = [EventTracingTestLogComing logComingWithRandomKey];
    [tester ne_et_asyncTapViewWithAccessibilityLabel:@"OK"];
    NE_ET_Test_WaitForTimeAtVTreeGenerateWithCondition(3, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_CLCK spm:alert_ok_btn_spm].count > 0;
    });
    
    VTree = logComing.lastVTree;
    NSArray<NSDictionary *> *alert_btn_ok_ec_logJsons = [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_CLCK spm:alert_ok_btn_spm];
    XCTAssertTrue(alert_btn_ok_ec_logJsons.count == 1);
    XCTAssertTrue([alert_btn_ok_ec_logJsons.firstObject[NE_ET_REFER_KEY_ACTSEQ] integerValue] == actseq + 1);      // 点击事件，声明了 userForRefer ，则会做 actseq ++
    actseq ++;
...
- (void)testSheet {
    ...
    NE_ET_Test_WaitForTimeAtVTreeGenerateWithCondition(3, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_VIEW spm:show_sheet_btn_spm].count > 0;
    });
    NE_ET_Test_WaitForTime(0.5);
    ...
    // 点击 show sheet 按钮
    [tester ne_et_asyncTapViewWithAccessibilityLabel:@"ShowSheet"];
    NE_ET_Test_WaitForTimeAtVTreeGenerateWithCondition(3, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_CLCK spm:show_sheet_btn_spm].count > 0;
    });
    ...
    NSArray<NSDictionary *> *sheet_pv_logJsons = [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_P_VIEW spm:sheet_spm];
    NSArray<NSDictionary *> *sheet_btn_handsome_ev_logJsons = [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_VIEW spm:handspme_btn_spm];
    NSArray<NSDictionary *> *sheet_btn_cancel_ev_logJsons = [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_VIEW spm:cancel_btn_spm];
    ...
    VTree = logComing.lastVTree;
    NEEventTracingVTreeNode *sheet_node = [self _findNodeBySPM:sheet_spm inVTree:VTree];
    NEEventTracingVTreeNode *sheet_btn_handsome_node = [self _findNodeBySPM:handspme_btn_spm inVTree:VTree];
    NEEventTracingVTreeNode *sheet_btn_cancel_node = [self _findNodeBySPM:cancel_btn_spm inVTree:VTree];
    ...
    XCTAssertTrue(sheet_btn_handsome_node.buildinEventLogDisableStrategy & NEETNodeBuildinEventLogDisableStrategyImpress);  // 禁止了曝光埋点
    ...
    XCTAssertTrue(sheet_btn_cancel_node.buildinEventLogDisableStrategy & NEETNodeBuildinEventLogDisableStrategyImpress);  // 禁止了曝光埋点
    XCTAssertTrue([sheet_btn_cancel_node.nodeParams.allKeys containsObject:@"sheet_btn_cancel_p_key"]);     // 对象参数
    
    // 点击 OK
    logComing = [EventTracingTestLogComing logComingWithRandomKey];
    [tester ne_et_asyncTapViewWithAccessibilityLabel:@"你很帅"];
    NE_ET_Test_WaitForTimeAtVTreeGenerateWithCondition(3, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_CLCK spm:handspme_btn_spm].count > 0;
    });
    
    VTree = logComing.lastVTree;
    NSArray<NSDictionary *> *sheet_btn_handsome_ec_logJsons = [logComing fetchJsonLogsForEvent:NE_ET_EVENT_ID_E_CLCK spm:handspme_btn_spm];
    XCTAssertTrue(sheet_btn_handsome_ec_logJsons.count == 1);
    XCTAssertTrue(![sheet_btn_handsome_ec_logJsons.firstObject.allKeys containsObject:NE_ET_REFER_KEY_ACTSEQ]);      // 点击事件，主动声明了不自增 actseq
...
- (NEEventTracingVTreeNode *)_findNodeBySPM:(NSString *)spm inVTree:(NEEventTracingVTree *)VTree {
