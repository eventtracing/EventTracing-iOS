...
- (void)testVisible {
    ...
    [tester et_asyncTapViewWithAccessibilityLabel:@"可见区域"];
    ...
    ET_Test_WaitForTimeAtVTreeGenerateWithCondition(3, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW spm:label_3_spm].count > 0;
    });
    ET_Test_WaitForTime(0.5);
    
    NSArray<NSDictionary *> *l1_ev_logJsons = [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW spm:@"label_1|page_visible"];
    NSArray<NSDictionary *> *l2_ev_logJsons = [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW spm:@"label_2|page_visible"];
    NSArray<NSDictionary *> *l3_ev_logJsons = [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW spm:@"label_3|page_visible"];
    NSArray<NSDictionary *> *l4_ev_logJsons = [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW spm:@"label_4|page_visible"];
    
    EventTracingVTree *VTree = logComing.lastVTree;
    
    // 不可见: 被父节点缩小后的可见区域裁剪掉了
    XCTAssertTrue(l1_ev_logJsons.count == 0);
    EventTracingVTreeNode *l1_node = [self _findNodeByOid:@"label_1" inVTree:VTree];
    ...
    EventTracingVTreeNode *l2_node = [self _findNodeByOid:@"label_2" inVTree:VTree];
    ...
    EventTracingVTreeNode *l3_node = [self _findNodeByOid:@"label_3" inVTree:VTree];
    ...
    EventTracingVTreeNode *l4_node = [self _findNodeByOid:@"label_4" inVTree:VTree];
    ...
    [tester tapViewWithAccessibilityLabel:@"曙光埋点"];
    {
        ...
        NSArray<NSDictionary *> *l2_ev_logJsons = [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW_END spm:@"label_2|page_visible"];
        XCTAssertTrue([l2_ev_logJsons.firstObject[ET_REFER_KEY_DURATION] integerValue] > 1000);
    }
...
- (void)testCustomControlVisibleOnLabel_3 {
    ...
    [tester et_asyncTapViewWithAccessibilityLabel:@"LogicalVisible"];
    ET_Test_WaitForTimeAtVTreeGenerateWithCondition(1, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW_END spm:label_3_spm].count > 0;
    });
    ...
    // 重新曝光 => _ev
    logComing = [EventTracingTestLogComing logComingWithRandomKey];
    [tester et_asyncTapViewWithAccessibilityLabel:@"SetNeedImpress"];
    ET_Test_WaitForTimeAtVTreeGenerateWithCondition(1, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW spm:label_3_spm].count > 0;
    });
    ...
    // hidden => _ed
    logComing = [EventTracingTestLogComing logComingWithRandomKey];
    [tester et_asyncTapViewWithAccessibilityLabel:@"Hidden"];
    ET_Test_WaitForTimeAtVTreeGenerateWithCondition(1, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW_END spm:label_3_spm].count > 0;
    });
    ...
    // alpha == 0 => _ed
    logComing = [EventTracingTestLogComing logComingWithRandomKey];
    [tester et_asyncTapViewWithAccessibilityLabel:@"Alpha"];
    ET_Test_WaitForTimeAtVTreeGenerateWithCondition(1, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW_END spm:label_3_spm].count > 0;
    });
...
- (void)testShowFloatView {
    ...
    logComing = [EventTracingTestLogComing logComingWithRandomKey];
    [tester et_asyncTapViewWithAccessibilityLabel:@"Show"];
    NSString *floatViewSPM = @"page_float_example_0|page_visible";
    ET_Test_WaitForTimeAtVTreeGenerateWithCondition(3, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:ET_EVENT_ID_P_VIEW spm:floatViewSPM].count > 0;
    });
    
    EventTracingVTree *VTree = logComing.lastVTree;
    NSString *floatViewCloseBtnSPM = @"btn_close|page_float_example_0|page_visible";
    NSString *l4_spm = @"label_4|page_visible";
    NSArray<NSDictionary *> *float_pv_logJsons = [logComing fetchJsonLogsForEvent:ET_EVENT_ID_P_VIEW spm:floatViewSPM];
    NSArray<NSDictionary *> *float_close_btn_ev_logJsons = [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW spm:floatViewCloseBtnSPM];
    ...
    NSArray<NSDictionary *> *l4_ev_logJsons = [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW spm:l4_spm];
    NSArray<NSDictionary *> *l4_ed_logJsons = [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW_END spm:l4_spm];
    XCTAssertTrue(l4_ev_logJsons.count == 0);
    XCTAssertTrue(l4_ed_logJsons.count == 1);
    
    EventTracingVTreeNode *root_page_node = [self _findNodeByOid:@"page_visible" inVTree:VTree];
    EventTracingVTreeNode *float_page_node = [self _findNodeByOid:@"page_float_example_0" inVTree:VTree];
    EventTracingVTreeNode *l4_node = [self _findNodeByOid:@"label_4" inVTree:VTree];
    EventTracingVTreeNode *l3_node = [self _findNodeByOid:@"label_3" inVTree:VTree];
    ...
    logComing = [EventTracingTestLogComing logComingWithRandomKey];
    [tester et_asyncTapViewWithAccessibilityLabel:@"Close"];
    NSString *label_4_spm = @"label_4|page_visible";
    ET_Test_WaitForTimeAtVTreeGenerateWithCondition(3, logComing, ^BOOL(EventTracingTestLogComing * _Nonnull logComing) {
        return [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW spm:label_4_spm].count > 0;
    });
    VTree = logComing.lastVTree;
    l4_ev_logJsons = [logComing fetchJsonLogsForEvent:ET_EVENT_ID_E_VIEW spm:l4_spm];
    XCTAssertTrue(l4_ev_logJsons.count == 1);
...
- (EventTracingVTreeNode *)_findNodeByOid:(NSString *)oid inVTree:(EventTracingVTree *)VTree {
